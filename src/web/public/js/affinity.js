...
function openDrawer(id) {
  ...
  content.innerHTML = `
    ...
    ${item.associatedArticles.length > 0 ? `<h3 class="member-heading associated-heading">Associerade artiklar <span>${item.associatedArticles.length}</span></h3><div class="drawer-articles associated-articles">${item.associatedArticles.map((article) => `<a class="drawer-article" href="/articles/${encodeURIComponent(article.articleNumber)}"><strong>${escape(article.articleNumber)}</strong><span>${escape(article.name)}</span><b>${escape(article.classification?.classification ?? "—")}</b><small>${format.format(article.currentPosition * 100)} %</small></a>`).join("")}</div>` : ""}
